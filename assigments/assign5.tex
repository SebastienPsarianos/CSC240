...
\null\qquad By the I.H: $N_q(x) = N_p(x)$\\
\null\qquad $N_q(qxp) = N_p(qxp) = N_q(x) + 1 = N_p(x) + 1$\\
\null\qquad $P(qxp)$\\
\null\quad Since $P(x)$ was assumed: $P(x)\Implies P(qxp)$\\\\
...
\null\qquad Let $b'$ be $b$ with an arbitrary $p$ and $q$ removed.\\
\null\qquad By the I.H: $(\forall x\in T.\left[|x|=2(n-1)\Implies Q(b)\right])$. Therefore $Q(b')$ \\
\null\qquad If the two removed characters are the first two\\
\null\qquad The constructor $pxqy$ allows for $pq$ to be concatinated to the start of $b'$ ($x=\lambda, y=b'$).\\
\null\qquad If the two removed characters are the first and last\\
\null\qquad The constructor $pxqy$ also allows for $b'$ to have $p$ concatnated at the start and $q$ \\
\null\qquad concatinated at the end ($x=b, y=\lambda$)\\
\null\qquad Since by the I.H every string in $T$ with a smaller cardinality than $b$ is in $R$, the\\
\null\qquad constructor $pxqy$ also allows for a $q$ to be placed in an arbitrary position by altering the \\
\null\qquad length of the strings $x$ and $y$\\
\null\qquad The same is possible with $p$ and $q$ in reverse order with use of the $qxpy$ constructor\\
\null\qquad With this combination it is possible to create any possible string $b\in T$\\
\null\quad Since $|b|=2n$ was assumed $|b|=2n\Implies Q(b)$
